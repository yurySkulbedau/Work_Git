...
Git добавляет к проблемным файлам стандартные метки, позволяющие открывать эти файлы вручную и разрешать конфликты. Для разрешения конфликта следует или выбрать одну из версий, или каким-то образом объединить их.  
...
>*git reset --hard ####* - откат к указанному коммитув рамках текущей ветки (по сути - отмена слияния).
